...
class Pod{
  ...
  waveRect(){return{x:this.x+S(9),y:this.y+S(31),w:this.w-S(18),h:max(S(78),this.h*.34)}}
  waveform(){
    ...
    noStroke();fill(C.amber);rect(lx-S(2),r.y,S(4),S(12));rect(lx+lw-S(2),r.y,S(4),S(12));
    ...
    let markers=max(1,floor(2+this.values.trans*22));
    for(let i=1;i<markers;i++){let mx=r.x+i/markers*r.w;stroke(i===this.selectedTransient?C.amber2:C.text3);line(mx,r.y+r.h-S(i===this.selectedTransient?10:6),mx,r.y+r.h)}
    ...
    textAlign(LEFT,BOTTOM);text('START',lx+S(5),r.y+r.h-S(4));textAlign(RIGHT,BOTTOM);text('END',lx+lw-S(5),r.y+r.h-S(4));
...
function drawFooter(){fill(C.text3);noStroke();textSize(S(6.8));textAlign(LEFT,BOTTOM);text('SHIFT + drag = precision   •   CTRL + wheel = waveform zoom   •   drag loop handles   •   click transient markers   •   drag master FX to reorder   •   drop audio on waveform',S(38),height-S(5));textAlign(LEFT,BASELINE)}
