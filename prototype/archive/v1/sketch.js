...
const C={
  ...
  blue:'#657078',wave:'#adb5ae',waveDim:'#6f7771'
};
...
function drawMainWindow(){
  ...
  fill(C.text3);textSize(S(8.5));text('six-pod probability sampler',x+S(64),y+S(17));
  textAlign(RIGHT,CENTER);text('standalone concept',x+w-S(14),y+S(17));
...
class Pod{
  constructor(i){
    ...
    this.wave=this.syntheticWave();this.actualWave=false;
  }
  syntheticWave(){
  ...
  waveRect(){return{x:this.x+S(9),y:this.y+S(31),w:this.w-S(18),h:max(S(64),this.h*.30)}}
  waveform(){
    const r=this.waveRect();fill(C.inset);stroke('#0b0c0c');rect(r.x,r.y,r.w,r.h,S(1));
    stroke(C.line2);strokeWeight(1);let grid=this.sync?16:8;
    for(let i=1;i<grid;i++){let gx=r.x+i/grid*r.w;line(gx,r.y,gx,r.y+r.h)}
    noStroke();fill(197,154,88,28);let lx=r.x+this.loopStart*r.w,lw=(this.loopEnd-this.loopStart)*r.w;rect(lx,r.y,lw,r.h);
    stroke(C.amber);line(lx,r.y,lx,r.y+r.h);line(lx+lw,r.y,lx+lw,r.y+r.h);
    let mid=r.y+r.h/2;stroke(this.actualWave?C.wave:C.waveDim);
    for(let i=0;i<this.wave.length;i++){let xx=r.x+i/(this.wave.length-1)*r.w,amp=this.wave[i]*r.h*.42;line(xx,mid-amp,xx,mid+amp)}
    if(!this.mute){let speedNorm=map(this.values.spd,0,1,.35,2.4);this.playhead+=.00085*speedNorm*(this.sync?1:.91);if(this.playhead>this.loopEnd)this.playhead=this.loopStart}
    let ph=r.x+this.playhead*r.w;stroke(C.amber2);line(ph,r.y,ph,r.y+r.h);
    stroke(C.text3);let markers=max(1,floor(2+this.values.trans*22));for(let i=1;i<markers;i++){let mx=r.x+i/markers*r.w;line(mx,r.y+r.h-S(6),mx,r.y+r.h)}
    noStroke();fill(C.text3);textSize(S(7));textAlign(LEFT,TOP);text(this.actualWave?'FILE':'DROP AUDIO',r.x+S(5),r.y+S(4));
    textAlign(RIGHT,BOTTOM);text(round(this.values.trans*100)+'% TRANS',r.x+r.w-S(5),r.y+r.h-S(4));
    textAlign(LEFT,BASELINE);miniButton(r.x+r.w-S(42),r.y+S(4),S(36),S(14),'PANIC',false,C.red);
  }
  mixer(){
  ...
  fxDrawer(){
    let x=this.x+S(7),y=this.y+this.h-S(104),w=this.w-S(14),h=S(72);fill('#222323');stroke(C.line);rect(x,y,w,h,S(1));
    let names=[['cut',this.values.filter],['sat',this.values.sat],['comp',this.values.comp],['low',this.values.eqLow],['mid',this.values.eqMid],['high',this.values.eqHigh]];
    for(let i=0;i<names.length;i++){let cx=x+S(28)+i*((w-S(56))/5);drawKnob(cx,y+S(30),S(20),names[i][1],0,1);
      fill(C.text3);noStroke();textSize(S(6.5));textAlign(CENTER,TOP);text(names[i][0],cx,y+S(43));}textAlign(LEFT,BASELINE);
  }
  press(mx,my){
  ...
  drag(mx,my){let d=activeDrag;if(!d||d.pod!==this)return;if(d.type==='slider')this.dragSlider(mx);if(d.type==='vol')this.dragVol(my);if(d.type==='pan'){let precision=keyIsDown(SHIFT),sens=precision?.002:.01;this.values.pan=constrain(d.startVal+(d.startMouse-my)*sens,-1,1)}}
...
class MasterFXChain{
  ...
  draw(){
    ...
    textAlign(RIGHT,BASELINE);text('click modules to bypass • reorderable in Max build',this.x+this.w-S(10),this.y+S(14));textAlign(LEFT,BASELINE);
    let sx=this.x+S(12),yy=this.y+S(24),gap=S(7),transportW=S(175),avail=this.w-S(24)-transportW,mw=(avail-gap*4)/5;
    for(let i=0;i<this.modules.length;i++)this.module(this.modules[i],sx+i*(mw+gap),yy,mw,S(66));
...
function mouseDragged(){bpmWin.drag(mouseX,mouseY);if(activeDrag?.pod)activeDrag.pod.drag(mouseX,mouseY)}
function mouseReleased(){bpmWin.release();activeDrag=null}
...
function drawFooter(){fill(C.text3);noStroke();textSize(S(6.8));textAlign(LEFT,BOTTOM);text('SHIFT + drag = precision   •   CTRL + wheel over waveform = zoom   •   drop audio directly on a waveform',S(38),height-S(5));textAlign(LEFT,BASELINE)}
